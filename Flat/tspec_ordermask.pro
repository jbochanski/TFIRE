FUNCTION TSPEC_ORDERMASK, tset_slits, FSR=fsr, CRYO_FACTOR=cryo_factor, WIDTH=width, LAM0=LAM0
;;
;; Optional inputs:
...
;; Cut off order 30,31 where they end
  ...
  if (not keyword_set(FSR)) then begin
     ord31mask = (ordermask NE 7) OR (yarr GT 660.0 AND ordermask EQ 7)
  endif else begin
