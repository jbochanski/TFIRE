function tspec_findslits, DATA=data, TRACEFLAT=traceflat, $
  ...
  func_name = "tspec_findslits.pro"
  if keyword_set(traceflat) then begin
     default_filename = TSPEC_GET_FILE_NAMES(traceflat, /ORDERFILE)
     default_str_filename = TSPEC_GET_FILE_NAMES(traceflat, /ORDR_STR_FILE)
  endif

  if keyword_set( data ) then begin
     if NOT keyword_set(orderfile) AND NOT keyword_set(TRACEFLAT) then begin
      orderfile = tspec_get(data,/ORDERFILE)
      ...
      ordr_str_file = tspec_get(data,/ORDR_STR_FILE)
    ...
    if NOT keyword_set(traceflat) then traceflat = tspec_get(data,/FLATS)
  endif
  ...
  tset_slits = tspec_traceorders(trcimg, /crude, ORDR_STR_FILE=ordr_str_file, ksize=13, CHK=CHK, INTER=INTER)
  ...
  ordermask=tspec_ordermask(tset_slits);, /fsr) 
   ...
   ;;ordermask[WHERE(slitmask GT 0)] = -slitmask[WHERE(slitmask GT 0)] + 21L
	...
	flat_str = tspec_names_to_list(traceflat)
