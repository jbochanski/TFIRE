...
;-
;------------------------------------------------------------------------------
FUNCTION tspec_traceorders, image $
                           , minslit = minslit $
                           ...
                           , CHK = CHK, INTER=INTER, FLATS=flats


func_name = "tspec_traceorders"
...
;	xright = [215, 325, 430, 540, 635, 725, 760, 840, 926, 1003, 1080, 1156, 1226, 1299, 1373, 1446, 1521, 1602, 1691, 1789, 1905]
	...
	fsum_trunc = fsum
	tspec_find_edges, fsum_trunc, xleft, xright, left_vals, right_vals, /SAW_TOOTH, LOUD=loud, NSECS=nsecs, NORD=norders, MIN_ORDER_WIDTH=min_order_width, DEBUG=debug, _EXTRA=keys
	...
	if n_elements(xleft) NE norders then begin
		...
		fsum_trunc = fsum
		fsum_trunc[where(abs(fsum_trunc) LT 10)] = 0

		x_fndpeaks, fsum_trunc, pks, autofit=fltarr(2048), /SILENT, /ALL, /THIN, rms = 100
		vals = fsum[pks]
		pksort = pks[sort(vals)];pks[reverse(sort(vals))]
		vals   = vals[sort(vals)];vals[reverse(sort(vals))]

		if (n_elements(pks) GT 5) then begin
		   xleft = pksort[0:4];-2
		   left_vals  = vals[0:4]
		endif
	endif
	...
	if n_elements(xright) NE norders then begin
		...
		fsum_trunc = fsum
		fsum_trunc[where(abs(fsum_trunc) LT 500)] = 0

		x_fndpeaks, -fsum_trunc, pks, autofit=fltarr(2048), /SILENT, /ALL, /THIN, rms = 100
		pks = pks[where(pks GT min(xleft))]
		vals = -fsum[pks]
		pksort = pks[sort(vals)]
		vals   = vals[sort(vals)]
		if (n_elements(pks) GT 5) then begin
		   xright = pksort[0:4]+2
		   right_vals   = vals[0:4]
