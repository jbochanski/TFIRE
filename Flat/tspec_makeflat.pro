pro tspec_makeflat, data=data, flatfiles=flatfiles, orders=orderfile, $
  ...
  if keyword_set( data ) then begin
    if NOT keyword_set( flatfiles ) then flatfiles = tspec_get(data,/FLATS)
    if NOT keyword_set( orderfile ) then orderfile = tspec_get(data,/ORDERFILE)
    if NOT keyword_set( scifile ) then scifile = tspec_get(data,/FITSFILE)
    if NOT keyword_set( arcfile ) then arcfile = scifile ;; Note: science file used here, not arc file!
    if NOT keyword_set( illumfiles ) then illumfiles = tspec_get(data,/ILLUMS)
    if NOT keyword_set( outfile ) then outfile = tspec_get(data,/ILLUMFLATFILE)
    if NOT keyword_set( pixflat ) then pixflat = tspec_get(data,/PIXFLATFILE)
    if NOT keyword_set( pixfile ) then pixfile = tspec_get(data,/PIXIMGFILE)
  endif
  ...
  if NOT keyword_set( outfile ) then begin
    outfile = TSPEC_GET_FILE_NAMES(illumfiles, /ILLUMFLAT) 
  endif
  if NOT keyword_set( pixflat ) then begin
    pixflat = TSPEC_GET_FILE_NAMES(flatfiles, /PIXFLATFILE) 
  endif
  if NOT keyword_set( pixfile ) then begin
		pixfile = TSPEC_GET_FILE_NAMES(flatfiles, /PIXIMGFILE) 
  endif
  ...
  if NOT keyword_set(CLOBBER) AND FILE_TEST(outfile) AND FILE_TEST(pixflat) AND FILE_TEST(pixfile) then begin
	...
	flat_str = tspec_names_to_list(flatfiles)
	illum_str = tspec_names_to_list(illumfiles)
  ...
  if NOT keyword_set(CLOBBER) AND FILE_TEST(pixfile) then begin
    ...
    piximg = tspec_makescipix(arcimg, tset_slits, pixset=pixset, chk=chk, arc2=arcimg)
...
;     mwrfits, tset_slits, orderfile
  ...
  if NOT keyword_set(CLOBBER) AND FILE_TEST(outfile) then begin
    ...
    ;if (n_elements(input) GT 1) then begin
	   illflat    = tspec_superfast_superflat(input, orderfile, pixfile  $
	                                       , illum=illumflat, /skyflat, $
	                                       chk=1, tweak=1,/noscat)
    ;endif else begin
  ...
  if NOT keyword_set(CLOBBER) AND FILE_TEST(pixflat) then begin
    ...
    input   =  flatfiles
    if (n_elements(input) GT 1) then begin
       flat    = tspec_superfast_superflat(input, orderfile, pixfile  $
                                       , chk=1)
    endif else begin
