function tspec_superfast_superflat, filenames, slitfile, pixfile $
                         , order_vec = order_vec, skyflat = SKYFLAT $
                         , NOSCAT = NOSCAT, ILLUM = ILLUMFINAL $
                         , TWEAK=TWEAK,TOL_EDG=TOL_EDG1,EDG_TRIM=EDG_TRIM1 $
                         , CHK=CHK
    
  ;;scattfile = strtrim(archive_path, 2)+'scatt_template.fits'
  
  prog_name = 'tspec_superfast_superflat';

  nflat = n_elements(filenames)
  IF nflat LT 1 THEN message, 'No flat field files given in fire_superfast_superflat'
...
;  endfor
  ...
  print, prog_name, ': Combining flat files using fire_combine_flats.pro...'
  TSPEC_COMBINE_FLATS, filenames, ordermask, edgmask, IMAGE=image, OUTMASK=outmask;, /ADD
...
;      stop
      ...
      IF (KEYWORD_SET(TWEAK)) THEN BEGIN
         ...
         IF xleft_tweak NE -1.0 THEN BEGIN
            ...
            splog, 'Tweaking position of LEFT boundary for order', 8L-iorder
         ENDIF
         ...
         IF xright_tweak NE -1.0 THEN BEGIN
           ...
           splog, 'Tweaking position of RIGHT boundary for order', 8L-iorder
         ENDIF  
      ENDIF
