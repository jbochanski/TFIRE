...
PRO tspec_find_edges, mid_line, front_edges, back_edges, front_vals, back_vals, SAW_TOOTH=saw_tooth, LOUD=loud, NSECS=nsecs, NORD=nord, MIN_ORDER_WIDTH=min_order_width, DEBUG=debug, _EXTRA=keys

	func_name = "tspec_find_edges"
...
nx = 900	
	...
	if NOT keyword_set(SAW_TOOTH) then begin
		...
		cutoff = 0.5*( noise_floor + sig_level )	
	endelse
...
cutoff = abs(cutoff)
	...
	bad_ords = where( widths LT min_order_width, nbad, COMPLEMENT=good_ords, NCOMPLEMENT=ngood )
