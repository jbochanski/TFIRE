...
function tspec_echjoefind, img, sciivar, waveimg,gdtrc, slit_edg $
                          ...
                          , MIN_SN = MIN_SN, CHK=CHK

	func_name = 'tspec_echjoefind'

  if  N_params() LT 4  then begin 
      print, 'Syntax - ' + $
             'ostr = tspec_echjoefind( img, var, gdtrc, slit_edg, cbin )  [v1.0]'
      return, -1
  endif 

  IF NOT KEYWORD_SET(NORDERS) THEN NORDERS=5;21L
  if not keyword_set(PEAKTHRESH) then peakthresh = 0
  if not keyword_set(STDSEP) then STDSEP = 2.0d;1.0d
  if not keyword_set(MIN_SN) then MIN_SN = 0.5
  IF NOT KEYWORD_SET(FWHM) THEN FWHM = 2.5
   ;; Fire pixels are 0.15". This FWHM presumes seeing is 0.6"
  cbin=1.0
  ...
  FOR iobj = 0L, nobj_str-1 DO BEGIN
     ...
     if nbad NE 0 then begin
        print, 'fire_echjoefind: Bad trace in slit ';, is
        msk[iobj] = 1 
     endif
  ENDFOR
...
;; boxcar extraction
     ...
     indsp_shf = WHERE(wave GT 800.0 AND wave LT 3.0d4 $
  ...
  IF nbad_sn NE 0 THEN BEGIN
     msk[bad] = 1
     print, 'esi_echjoefind: S/N ratio too low in orders:' 
     ;forprint,'                                 ' + $
     ;         string(31 - (objstruct[bad].slitid - 1)),textout=1
     print,'                                 ' + $
              fire_string(7 - (objstruct[bad].slitid - 1))    
  ENDIF
  spots = where(msk EQ 0, nspots);
  if nspots EQ 0 then begin
  		fire_siren, func_name + ":  ERROR! SNR too low in all orders!  Returning non-sensical value!"
  		RETURN, -1
  endif
  ...
  for kk=norders,1,-1 do begin
      for ss=0L,nobj-1 do begin
          ...
          if nmt EQ 0 then begin ;; Nope
             print, 'fire_echjoefind: Extrapolating object ', lbl[ss], ' in '+$
                    'order ',8-(kk-1)
  ...
  if keyword_set(NFIND) then begin
      if NOBJ LT NFIND then begin
          print, 'fire_echjoefind:  Found fewer objects than you desired.', $
                 nfind, nobj
          print, 'fire_echjoefind:  Consider setting MINPEAK to a value lower'
          print, 'fire_echjoefind:  than ', MINPEAK
          nfind = nobj
      endif
  endif
