...
function tspec_echextobj,SCIIMG=sciimg,SCIIVAR=sciivar,SCIHDR=scihdr,SKYIMAGE=skyimage $
                   ...
                   , BOX_RAD=BOX_RAD1,STD=STD,CHK=CHK, CHECK_INPUTS=check_inputs $
                   , AIRVAC_CORR=AIRVAC_CORR, HELIO_CORR=HELIO_CORR, REDUXDIR=reduxdir $
                   , BRIGHT=BRIGHT, SPCCHK=SPCCHK, DEBUG=DEBUG, VERBOSE=verbose, OBJNAME=objname

;, esi, obj_id, exp, DEBUG=debug, CHK=chk $
...
;      print,'Syntax - ' + $
;        'fire_echextobj, fire, obj_id, [exspr], RADIUS=, APER=, /DEBUG, /CHK'
...
;  endif 
  ...
  if keyword_set(DATA) then begin
     if NOT keyword_set(objstr) then begin
        if keyword_set(GENERATE) then begin
           ...
           objstr = tspec_findobj( sciimg-skyimage, sciivar, waveimg, tset_slits $
     ...
     if NOT keyword_set(skyimage) then begin
        if keyword_set(GENERATE) then begin
           ...
           skyimage = tspec_skymodel(DATA=data, SCIIMG=sciimg, SCIIVAR=sciivar, HDR=scihdr, STD=std, VERBOSE=verbose, CHECK_INPUTS=check_inputs)
        endif else if NOT keyword_set(CHECK_INPUTS) then begin
     ...
     if NOT keyword_set(sciimg) OR NOT keyword_set(SCIIVAR) OR NOT keyword_set(SCIHDR) then begin
        ...
        sciimg = tspec_get(data, /SCIIMG, STD=std, SCIIVAR=sciivar, HDR=scihdr, VERBOSE=verbose, FILEREAD=sciimgfile)
     endif
     if NOT keyword_set(piximg) then begin
        print, func_name, ": Generating piximg..."
        piximg = tspec_get(data, /PIXIMAGE, VERBOSE=verbose, FILEREAD=piximgfile)
     endif
     if NOT keyword_set(waveimg) then begin
        print, func_name, ": Generating waveimg..."
        waveimg = tspec_get(data, /WAVEIMG, VERBOSE=verbose, FILEREAD=waveimgfile)
        waveimg = 10^waveimg
     endif
     if NOT keyword_set(tset_slits) then begin
        print, func_name, ": Generating tset_slits..."
        tset_slits = tspec_get(data, /TSET_SLITS, VERBOSE=verbose, FILEREAD=tset_slitsfile)
     endif
     if NOT keyword_set(ECHEXTFILE) then begin
        echextfile = tspec_get(data, /ECHEXTFILE, STD=std, VERBOSE=verbose)
     endif
     if NOT keyword_set(OBJFILE) then begin
        objfile = tspec_get(data, /OBJSTRFILE, STD=std, VERBOSE=verbose)
     endif
  endif
  ...
  wave_ordr = [9380,11170,13095,16350,21770]*1.
...
;;  Wavelength solution (put in a file)
  cdelt = 30.5d/299792.458d/alog(10.0d);12.5d/299792.458d/alog(10.0d)
  npix = 3000 
;;  cdelt = 0.00001447624d
  tot_wave = 10^(alog10(8000.d) + dindgen(10240L)*cdelt);dindgen(37720L)*cdelt)
  all_crval1 = dblarr(norders)
  all_crval1[0] = 8120.0;8240.0
  all_crval1[1] = 9440.0;8350.0
  all_crval1[2] = 11320.0;8550.0
  all_crval1[3] = 14140.0;8800.0
  all_crval1[4] = 18820.0;9150.0
;  all_crval1[5] = 9500.0
;  all_crval1[6] = 9900.0
...
;  all_crval1[20] = 22000.0
  ...
  all_coll[0,*] = [8120.,10640.];[8240., 8900.]
  all_coll[1,*] = [9440.,12400.];[8300., 9200.]
  all_coll[2,*] = [11320.,14870];[8900., 9600.]
  all_coll[3,*] = [14140.,18560.];[9000., 9900.]
  all_coll[4,*] = [18820.,24720.];[9150., 10300.]
...
;  all_coll[20,*] = [21100., 25000.]
  ...
  IF djs_median(waveimg[where(waveimg GT 1.0)]) LE 10.0 THEN BEGIN
  ...
  if keyword_set(HELIO_CORR) then begin
     ...
     jd = sjs_apo_hdrget(scihdr,/rjd);fire_get_jd(scihdr, /REDUCED) ; Geocentric JD-2400000 (ie, 'reduced' Julian date)
     ra_deg = sjs_apo_hdrget(scihdr,/ra);sxpar(scihdr, "RA")
     dec_deg = sjs_apo_hdrget(scihdr,/dec);sxpar(scihdr, "DEC")
    ...
    ; Heliocentric velocity correction in km/s
     ...
     helio_corr = (-1.0)*x_apohelio(ra_deg, dec_deg, jd=jdhelio, obs='apo')
     gamma = sqrt( (1.d + helio_corr/299792.458d) / $
                   (1.d - helio_corr/299792.458d))
     waveimg = waveimg * gamma
     
     msg = 'Helio corr (NO vacuum) applied for '+sxpar(scihdr,"OBJNAME")+', helio = '+strtrim(helio_corr,2)+' km/s'
     print, msg
...
;;
  ...
  FOR iobj = 0L, nobj-1L DO BEGIN
     FOR iorder = 0L, nord-1L DO BEGIN
        ...
        ;; mask
        snmask = ordermask EQ (7-qq) AND sciivar GT 0.0 AND waveimg GT 0.0
        ;; boxcar extraction
  ...
  print, transpose ([[string(7L-ordrs[srt_s2n], FORMAT='(I6)')] ,  [ string(sqrt(ordr_s2n[srt_s2n, 0]), FORMAT='(F10.2)') ] ] )  
  splog, '*************************************************'
  FOR jorder = 0L, nord-1L DO BEGIN
     ...
     thismask = (ordermask EQ (7-qq)) AND waveimg GT 0.0
        ...
        IF ordr_s2n[iorder, iobj] LE LOWSNR AND nother GT 2 THEN BEGIN
           ...
           IF isrt EQ 0 THEN BEGIN
              ...
              ;          , format = '(I6,F10.2)', textout = 1              
				  ; forprint causes the code to halt until user hits spacebar if the terminal window is too small
				  ; the following print line does not, although it is certainly uglier...
				  print, transpose ( [ [ string(7L-ordrs[other_orders], FORMAT='(I6)')] ,  [ string(fwhm_here[other_orders], FORMAT='(F10.2)') ] ] )                
              
              

              fwhm_coeffs = ladfit(7-ordrs[other_orders] $
                                   , fwhm_here[other_orders])
              def_fwhm = poly(7L-qq, fwhm_coeffs)
              def_fwhm = def_fwhm[0]
              fwhm_was_fit[iorder] = 1
              splog, 'FIT: ' + string(7L-qq, format = '(I4)') $
                     + string(def_fwhm, format = '(F10.2)')
              splog, '*************************************************'
              if NOT keyword_set(NOPLOT) then begin
	              wset, 1
	              plot, 7L-ordrs[other_orders], fwhm_here[other_orders] $
	                    , psym = 4, symsize = 2, xr = [10.0, 33.0] $
	                    , yrang = [0.7*min([fwhm_here, def_fwhm]) $
	                               , 1.4*max([fwhm_here, def_fwhm])] $
	                    , /xsty, /ysty
	              clr = getcolor(/load)
	              IF nfit GT 0 THEN oplot, 7L-ordrs[other_fit] $
	                                       , fwhm_here[other_fit], psym = 6, symsize = 2 $
	                                       , col = clr.cyan
	              oplot, [7L-qq], [def_fwhm], psym = 5, symsize = 2 $
	                     , col = clr.green
	              oplot, 7L-ordrs, poly(31L-ordrs, fwhm_coeffs) $
	                     , col = clr.red
	              wset, 0
					endif
     ...
     IF NOT KEYWORD_SET(BOXCAR) THEN BEGIN
        extract_struct = tspec_localskysub(sciimg, sciivar, skyimage $
     ...
     FOR isrt = 0L, nobj-1L DO BEGIN
        ...
        ENDIF ELSE BEGIN ;; BOXCAR EXTRACTION
           ...
           f_extobjbox, timg-tsky, tarc, VAR = tvar $
                        ...
                        ;; range of the order
                        , TOT_TRC = ( (objstr[sci2].trace[0:ny-1L]-mincol) > 0.0 ) $
                        , REJSIG = rejsig, /REBINC, RADIUS = box_rad $
                        , NOCRREJ = NOCRREJ $
                        , BKAPER = svaper, FLAG=flag, CHK = chk
           if flag NE 0 then begin
           		fire_siren, func_name + ": ERROR! f_extobjbox failed!  Exiting."
           		RETURN, -1
           endif
           ;; Save the aperture
  ...
  sxdelpar, objhdr, "SIMPLE" ;xx
  sxdelpar, objhdr, "PCOUNT"
  sxdelpar, objhdr, "GCOUNT"
  sxdelpar, objhdr, "EXTEND"
