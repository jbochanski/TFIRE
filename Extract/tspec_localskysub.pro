...
;   August 2010  Adapted for FIRE, derived from long_localskysub
;-  
;------------------------------------------------------------------------------

function tspec_localskysub, sciimg, sciivar, skyimage, piximg, waveimg  $
                           ...
                           , sigrej = sigrej1, PROF_NSIGMA=prof_nsigma $
                           , NOLOCAL=nolocal, OBJNAME=objname, NOPLOT=noplot
...
;;    keep for next iteration
      ...
      if nc GT 100 then npoly = 2L $
      else if nc GT 40 then npoly = 1L $
      else npoly = 1L
        ...
        FOR ii = 0L, objwork-1L DO BEGIN
            ...
            ENDIF ELSE BEGIN
                ...
                temp_str = long_extract_optimal(thismask*waveimg $
                                                ...
                                                , rn_img = rn_img)
                ;; Bad extraction?
                if total(temp_str.wave_opt[0]) GT 0. then begin
                    ;; Good extraction
             ...
             ENDELSE
;            if (i1 EQ 19) then stop
            if (total(wave) LE 0) then stop
            obj_profiles[ipix, ii] = fire_gprofile(img_minsky[ipix] $
                                                   , (modelivar*mask)[ipix] $
                                                   , waveimg[ipix] $         
                                                   , objstruct[indx[iobj]].xpos $
                                                   - mincol $
                                                   ...
                                                   , MED_SN2 = MED_SN2, OBJNAME=objname $
                                                   , NOPLOT=noplot, /silent)
...
;       Use spatial profile of the sky to determine the SKY_MODEL
        sky_bmodel = 0.0
        iterbsp = 0
        WHILE total(sky_bmodel) EQ 0.0 AND iterbsp LE 5 and not keyword_set(NOLOCAL) DO BEGIN
...
;                  AND obj_profiles[*, zz] LT 0.05*max(obj_profiles[*, zz])
            ...
            ENDIF
         ENDWHILE
         rn_img = fltarr(nx,ny);long_rdnoiseimg(nx, ny, scihdr)
        ...
        if not keyword_set(NOLOCAL) then begin
            ...
            IF ngd GT 0 THEN BEGIN
                ...
                sigrej_eff = (sqrt(chi2_sigrej) >  sigrej) <  10.0D
   ...
   for ii = 0L, objwork-1L do begin
       ...
       objmask =  ((xarr GE (trace - 2.0*box_rad)) AND $
                   (xarr LE (trace + 2.0*box_rad)))
       single_struct = long_extract_optimal(thismask*waveimg $
                                            , img_minsky $
