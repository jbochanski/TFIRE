...
pro f_extapprof, fx, wv, xyguess, MSK=msk, VAR=var, $
	...
	func_name = "f_extapprof"

;  Error catching
  if  N_params() LT 3  then begin 
    print,'Syntax - ' + $
             'f_extapprof, fx, wv, xyguess, MSK=, VAR=, '
...
; Collapse along the trace (restrict to 3200 - 8000A by default)
  ...
  collpix = where(wv[lindgen(sz[0]),rnd_trc] GT collmnx[0] AND $
	...
	if ncoll EQ 0 then begin
		fire_siren, func_name + ": ERROR collapsing trace!  Exiting."
		flg_aper = -1
		return
	endif
...
; Spline the aperture
  ...
  if flg_smsh EQ 1 then begin
      ...
      gfit = gaussfit(ax[indfit], ay[indfit], gcoeff $
                      , nterms = 3) ;, sigma=gsig)
