...
FUNCTION tspec_findobj, IMG_MINSKY=img_minsky,SCIIVAR=sciivar,$
  ...
  if keyword_set(DATA) then begin
     if NOT keyword_set(img_minsky) then begin
        if keyword_set(GENERATE) then begin
           ...
           skyimage = tspec_skymodel(DATA=data, SCIIMG=sciimg, SCIIVAR=sciivar, STD=std, VERBOSE=verbose)
     ...
     if NOT keyword_set(sciivar) then begin
        ...
        sciimg = tspec_get(data, /SCIIMG, STD=std, SCIIVAR=sciivar, VERBOSE=verbose, FILEREAD=sciivarfile)
     endif
     if NOT keyword_set(waveimg) then begin
        print, func_name, ": Generating waveimg..."
        waveimg = tspec_get(data, /WAVEIMG, STD=std, VERBOSE=verbose, FILEREAD=waveimgfile)
        waveimg = 10^waveimg
     endif
...
;;  Optional Keywords
  if not KEYWORD_SET(NORDERS) THEN NORDERS=5;21
