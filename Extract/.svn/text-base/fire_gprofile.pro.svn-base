...
;-  
;------------------------------------------------------------------------------
FUNCTION fire_gprofile, image, ivar, waveimg $
                        ...
                        , NO_DERIV=no_deriv, OBJNAME=objname, NOPLOT=noplot
...
IF NOT KEYWORD_SET(OBJNAME) then objname = 'Object ' + fire_string(objstruct.objid)

if NOT keyword_set(objstruct) then title_string = '' else $
  title_string = objname + ' Slit ' + fire_string(objstruct.slitid)
...
indsp = WHERE(wave GT 2900.0 AND wave LT 3.0d4 $
...
norm_obj = float(sub_obj/(spline_sub + (spline_sub EQ 0.0)))
...
if ngood LT 10  OR med_sn2 LT SN_GAUSS^2 OR KEYWORD_SET(GAUSS) then begin
    ...
    if NOT keyword_set(NOPLOT) then begin
	    IF ngood GT 0 THEN $
	       qa_longslit_profile, (sigma_x[good] < 7) > (-7) $
	                            , norm_obj[good], profile_model[isub[good]] $
	                            , title = title_string $
	    ELSE qa_longslit_profile, (sigma_x < 7) > (-7) $
	                              , norm_obj, profile_model[isub] $
	                              , title = title_string + $
...
GOOD_PIX = sn2_sub GT 3.0D AND norm_ivar GT 0
...
title_string = title_string  $
  + ' FWHM range: ('  + fire_string(min(fwhmfit), FORMAT='(F6.2)') + ', ' + $ 
  fire_string(max(fwhmfit), FORMAT='(F6.2)') + $
  ') S/N: '+ fire_string(sqrt(med_sn2), FORMAT = '(F8.1)') $
  + ' Chi^2: '+ fire_string(chi_med, FORMAT = '(F8.2)') + fire_string(chi_zero, FORMAT = '(F8.2)') 
x1 = sigma_x[in]
...
profile_2d = bspline_iterfit(sigma_x[sf], norm_obj[sf], $
                             ...
                             npoly = 5, x2 = xtemp[sf] $
                             , /groupbadpix, maxrej = 5, upper = 10, lower = 10, /silent)
