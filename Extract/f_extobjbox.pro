...
; CALLING SEQUENCE:
;   
;  f_extobjbox, fx, wv, xyguess, fin_spec, MSK=, VAR=, WVMNX=
;
; INPUTS:
;  fx  -- Image array
;  wv  -- Wavelength array
...
;	 FLAG  -   0 if successful, non-zero otherwise
;
; OPTIONAL OUTPUTS:
;
; COMMENTS:
...
pro f_extobjbox, fx, wv, xyguess, fin_spec, MSK=msk, VAR=var, WVMNX=wvmnx, $
                 ...
                 REBINC=rebinc, BKAPER=bkaper, SIG_COLL=sig_coll, $
                 SILENT=silent, FLAG=flag, CHK=chk

	func_name = "f_extobjbox"
	flag = 0

;  Error catching
  if  N_params() LT 4  then begin 
    print,'Syntax - ' + $
             'f_extobjbox, fx, wv, xyguess, fin_spec, MSK=, VAR=, '
...
;  Optional Keywords
  ...
  if not keyword_set( APER ) then begin
      ;; Get the aperture
      f_extapprof, fx, wv, xyguess, APSTRCT=apstrct, FLG_APER=flg_aper, MSK=msk,$
      ...
      if flg_aper EQ -1 then begin
          fire_siren, func_name + ": Failure with f_extapprof!  Exiting.'
          fin_spec = { npix: 0 }
          flag = -1
          return
      endif
      if apstrct.flg_smsh NE 1 then begin
          ...
          print, func_name + "Warning:  No flux to set aperture for! Using backup!"
      endif
  endif else begin
      print, func_name + ": Using an aperture ", aper
      apstrct = { $
                aper: aper, $
                gcoeff: fltarr(3), $
                flg_smsh: 0 $
