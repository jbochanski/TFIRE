...
FUNCTION FIRE_EXPTYPE_MATCH, file, old_value, new_value, LOUD=loud
  ...
  if strmatch(new_value, "ARC", /FOLD_CASE) then begin
     if strmatch(old_value, "ARC", /FOLD_CASE) OR strmatch(old_value, "ARCFILE", /FOLD_CASE) OR $
        ...
        strmatch(old_value, "FIRESTARTER", /FOLD_CASE) then begin
        match = 1
     endif else begin
        match = 0
     endelse
...
;    Exposure types may be the following (needs to be updated): 
...
PRO FIRE_GUESS_EXPTYPE, data, LOUD=loud, VERBOSE=verbose, $
  ...
  for ifile=0, nfiles-1 do begin
     ...
     if lamp EQ 'ThAr' then begin
        if mirror EQ 'Clear' then begin
           print, func_name, ": problem with data file " $
                  + file + $
                  "--> ThAr lamp on, but mirror is clear!  Data is junk!"
           fire_append_warning, data, ifile, "ThAr lamp but no mirror"
           guess = "JUNK"
        endif else begin
           data[ifile].object = "ThAr"
           guess = "ARC"
        endelse
        data[ifile].exptype = FIRE_CHANGE_EXPTYPE_VAL(func_name, file, "EXPTYPE", exptype, $
                                                      guess, INTERACTIVE=interactive, LOUD=loud, $
...
PRO FIRE_MATCH_ARCS, data, LOUD=loud, INTERACTIVE=interactive, VERBOSE=verbose
  ...
  max_ang_off = 0.05 ;; maximum allowed offset in degrees
...
;; in/astro/users/sjschmidt/pro/FIRE/Utils/update_firestrct_tell_info.proputs an array of firestrct, and matches all science objects and tellurics within that array to arcfiles within that array
...
;           stop
           ...
           data[ spot ].tfiles = tspec_names_to_list(bestfiles)
  ...
  if is_undefined(flatfiles) EQ 1 then begin
     ...
     for i =0, nflatfiles-1 do begin
        ...
        if nmatch EQ 0 then begin
           ...
           flatnums1 = FIX(TSPEC_GET_FITNUM(data[flats].fitsfile))
...
PRO FIRE_MATCH_FLATS, data, PIXFLATS=pixflats, ILLUMFLATS=illumflats, $
	...
	flats = where( strpos(strupcase(data.exptype), 'FLAT') NE -1)
	flatnums = FIX(TSPEC_GET_FITNUM(data[flats].fitsfile))
...
PRO TSPEC_MKSTRCT, data, RAWPATH=rawpath, VERBOSE=verbose, $
...
;     endif
     ...
     a.exptime = sxpar(hdr, "EXPTIME");fire_get(a, /EXPTIME, /QUIET)
;     object = fire_get(a, /OBJECT, /QUIET)
     words = strtrim(strsplit(files[ifile],'/',/extract,count=nword),2)
     a.object = (strtrim(strsplit(words[nword-1],'.',/extract),2))[0]
     ra_sex = double(strsplit(sxpar(hdr, "RA"),':',/extract))
     dec_sex = double(strsplit(sxpar(hdr, "DEC"),':',/extract))
     if n_elements(ra_sex) eq 1 then begin
       a.ra_deg = 0
       a.dec_deg = 0
     endif else begin
       a.ra_deg = (ra_sex[0] + (ra_sex[1] + ra_sex[2]/60.)/60.)*15.
       if abs(dec_sex[0]) eq dec_sex[0] then $
         a.dec_deg = dec_sex[0] + (dec_sex[1] + dec_sex[2]/60.)/60. else $
         a.dec_deg = -(abs(dec_sex[0]) + (dec_sex[1] + dec_sex[2]/60.)/60.)
     endelse
     a.airmass = sxpar(hdr, "AIRMASS") ;fire_get(a, /AIRMASS, /QUIET)
     a.exptype     = sxpar(hdr, "IMAGETYP");fire_get(a, /EXPTYPE, /QUIET)
     date = strsplit(sxpar(hdr,"DATE-OBS"),':-TO',/extract)
     a.jd = julday(date[1],date[2],date[0],date[3],date[4],date[5]);fire_get(a, /JD, /QUIET)
...
;     endelse
     ...
     if strmid(a.object,0,2) eq '2M' or strmid(a.object,0,2) eq 'SD' or $
       strmid(a.object,0,1) eq 'L' or strmid(a.object,0,1) eq 'D' then $
       a.exptype = 'Science' else a.exptype = 'Tell'
     
     if a.object eq 'flat' or a.object eq 'Flat' then a.exptype = 'FLAT'
...
;added a set of rules for my tspec stuff above
...
;  FIRE_MATCH_ARCS, data, LOUD=loud, INTERACTIVE=interactive
  ...
  ;; Determine wavelength calibration method (OH lines or ThAr) for all science files
	scis = fire_get_scis(data, spots=spots, NSPOTS=nspots)
	min_time_for_OH = 6
	if nspots GT 0 then begin
		OH_spots = where( data(spots).exptime GE min_time_for_OH, noh, COMPLEMENT=THAR_spots, NCOMPLEMENT=nthar )
		if noh GT 0 then begin
			data( spots(OH_spots) ).OH_cal = 1
		endif
		if nthar GT 0 then begin
			data( spots(THAR_spots) ).OH_cal = 0
		endif
	endif
    
  ;; Fill in the names of the object structure files
  TSPECSTRCT_FILL_FILENAMES, data, /OBJSTRFILE, /QUIET
  ...
  update_tspecstrct_tell_info, data
