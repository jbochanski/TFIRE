...
common fire_editstrct_common
  ...
  type = strtrim(x_guilist(['LD','FLAT','SCIENCE','ARC','TELL','Unknown']),2)
...
common fire_editstrct_common
  ...
  if (is_empty(tels)) then begin
     ...
     ;; Change
     cmm_fire[cmm_indx].tfiles = tspec_names_to_list(tels)
...
common fire_editstrct_common
  ...
  cmm_fire[cmm_indx].arcs = tspec_names_to_list(newarc)
...
pro tspec_editstrct, firestrct, fitsname=fitsname, raw=raw
...
; Finish
  ...
  update_tspecstrct_tell_info, fire  
