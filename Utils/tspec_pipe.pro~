...
pro tspec_pipe, fire, targets=targetnames, clobber=clobber, $
  ...
  for itarg=0, ntargs-1 do begin
     ...
     for iframe=0, nmatch-1 do begin
        ...
        rawpath = tspec_get(scistruct, /rawpath)
        scifil1 = strtrim(tspec_get(scistruct, /fitsfile, /donotcalc),2)
        scifil  = rawpath + scifil1
        pixflat = tspec_get(scistruct, /Pixflatfile)
        illum   = tspec_get(scistruct, /Illumflatfile)
        
        tell    = tspec_get(scistruct, /TFiles)
        ...
        ordrfil = tspec_get(scistruct, /Orderfile)
        ostrfil = tspec_get(scistruct, /Ordr_str_file)
        
        objname = tspec_get(scistruct, /Object) + " (" + tspec_get_fitnum(scifil1) + ")"
        ...
        exptime = tspec_get(scistruct, /Exptime)			
        ...
        if keyword_set(OH) then begin
           wcfile = rawpath + tspec_get(scistruct, /WAVECALFILE, /OH) ;; wavelength calibration filename
           wcfits = tspec_get(scistruct, /ARCFITS, /OH) ;; output wavelength calibration fits filename
           wcimg = tspec_get(scistruct, /ARCIMGFITS, /OH) ;; output wavelength calibration image filename
        endif else if keyword_set(THAR) then begin
           wcfile = tspec_get(scistruct, /WAVECALFILE, /THAR)           ;; wavelength calibration file
           wcfits = tspec_get(scistruct, /ARCFITS, /THAR) ;; output wavelength calibration fits filename
           wcimg = tspec_get(scistruct, /ARCIMGFITS, /THAR) ;; output wavelength calibration image filename
        endif else begin
...
;                        CLOBBER=clobber, CHK=chk
              ...
              result=tspec_arc2(arcraw = wcfile, ordr_str=mrdfits(ostrfil,1), $
...
;                        /OH, CLOBBER=clobber, CHK=chk
              ...
              result=tspec_arc2(arcraw = strtrim(wcfile,2), $
                         ordr_str=mrdfits(strtrim(ostrfil,2),1), $
        ...
        ;; Determine the important file names
        objstrfil = tspec_get(scistruct, /OBJSTRFILE)
        outfil = tspec_get(scistruct, /ECHEXTFILE) 
        ...
        if (x_chkfil(objstrfil, /SILENT) EQ 0) OR keyword_set(CLOBBER) then begin
           ...
           ;; Trim, flat field, etc
           tspec_proc, strtrim(scifil,2), sciimg, sciivar, hdr=scihdr, $
           ...
           if keyword_set(THAR) then begin
              tspec_proc, wcfile, arcimg, arcivar, hdr=archdr, $
                         pixflatfile=pixflat,$
                         illumflatfile=illum, /MASKBADPIX
              piximg = tspec_makescipix(arcimg, tset_slits, arc2=arcimg, CHK=chk)
              
           ;; If we're instead using OH lines, then pull piximg from the science file
           endif else begin
;stop
;              thar_fits = tspec_get(scistruct, /ARCS)
...
;              fire_proc, thar_fits, arcimg, arcivar, hdr=archdr, $
             tspec_proc, strtrim(scifil,2), arcimg, arcivar, hdr=archdr, $
                         pixflatfile=pixflat,$
                         illumflatfile=illum, /MASKBADPIX
              piximg = tspec_makescipix(sciimg, tset_slits, arc2=arcimg, CHK=chk)
           endelse
				...
				if size(piximg, /type) EQ 2 then begin
	         	...
	         	continue
                                ...
                                ; Fit the 2D sky
           skyimage = tspec_skymodel(sciimg=sciimg, sciivar=sciivar, $
                                    piximage=piximg $
           ...
           obj_strct = tspec_findobj(img_minsky=imgminsky,$
           ...
           if (NOT keyword_set(BRIGHT)) then begin
              result = tspec_echextobj(sciimg=sciimg, sciivar=sciivar, scihdr=scihdr, $
                              skyimage=skyimage,$
                              ...
                              CHK=1, /HELIO_CORR, OBJNAME=objname)
           endif else begin
     ...
     ;;;;;;;;;;; CALIBRATE AND EXTRACT TELLURICS ;;;;;;;;;;;;;
        ...
        if bad_tells EQ 1 then begin
         ...
         fire_pipe_widget_status, "  Extracting " + fire_string(ntells) $
               ...
               fire_str_array_to_str(fire_string(tspec_get_fitnum(tell)) ) + ")", $
           		VERBOSE=verbose, WIDGET=widget, _EXTRA = keys
...
;; he or she wants a single telluric file re-done once, not once for every science exposure input.
           if is_undefined(tellsdone) EQ 0 then begin
              if max( strmatch(tellsdone, telfile) ) EQ 1 then begin
                 msg = "Skipping telluric file " + tpsec_get_fitnum(telfile) + " (" + fire_string(itel+1) + $
           ...
           tarc   = (tspec_get(scistruct, /TARCS))[0]
...
;; Determine the important file names
           ...
           ordrfil = tspec_get(scistruct, /Orderfile)
           ostrfil = tspec_get(scistruct, /Ordr_str_file)
           twcfits = tspec_get(scistruct, /ARCFITS, /THAR, STD=itel+1, /DONOTSET)                    ;; output wavelength calibration fits filename
           twcimg  = tspec_get(scistruct, /ARCIMGFITS, /THAR, STD=itel+1, /DONOTSET)                 ;; output wavelength calibration image filename
           tobjname = tspec_get(scistruct, /TELLNAME) + " (" + tspec_get_fitnum( telfile ) + ")"
	...
	;; Determine the name of the output structure file
           objstrfil = tspec_get(scistruct, /TOBJSTRFILE, STD=itel+1)
           ...
           if (x_chkfil(objstrfil, /SILENT) NE 0) AND NOT keyword_set(CLOBBER) then begin
              print, func_name + ": File " + objstrfil + $
               ...
               fire_pipe_widget_status, "    ...done extracting telluric file " + tspec_get_fitnum(telfile) + " (" + $
...
;							lampon = strtrim(sxpar(headfits(tarc),"LAMP"),2) EQ 'ThAr'
							...
							if (lampon) then begin
								result = tspec_arc2(arcraw = tarc, ordr_str=mrdfits(ostrfil,1), $
								arcfits=twcfits, NOPLOT=noplot, $
								arcimgfits=twcimg, reduxpath=reduxpath, $
									/OH, CHK=chk, CLOBBER=clobber, OBJNAME=tobjname $;/ThAr, CHK=chk, CLOBBER=clobber, OBJNAME=tobjname $
           ...
           ; NOW PROCESS AND EXTRACT
				...
				;; Determine the important file names
           objstrfil = tspec_get(scistruct, /TOBJSTRFILE, STD=itel+1)
           outfil = tspec_get(scistruct, /TECHEXTFILE, STD=itel+1) 
           outfilzip = outfil + '.gz' ;; doesn't appear to be used...
           ...
           if (x_chkfil(objstrfil, /SILENT) EQ 0) OR keyword_set(CLOBBER) then begin
                                ...
                                ; Trim, flat field, etc
              tspec_proc, telfile, sciimg, sciivar, hdr=scihdr, $
                         pixflatfile=pixflat,$
                         illumflatfile=illum, /MASKBADPIX
              ...
              if ( is_empty(tarc) EQ 1 ) then begin
                 ...
                 tspec_proc, wcfile, wcimg, wcimgivar, hdr=wcimghdr, $
                            pixflatfile=pixflat,$
                            illumflatfile=illum, /MASKBADPIX

                 piximg = tspec_makescipix(wcimg, tset_slits, arc2=wcimg, CHK=chk) 
              endif else begin
                 ; Always use the ThAr arc for telluric calibrations
                 ; The sky is too faint for ~1 second exposures.
                 tspec_proc, tarc, tmparcimg, tmparcivar, hdr=tmparchdr, $
                            pixflatfile=pixflat,$
                            illumflatfile=illum, /MASKBADPIX
                 piximg = tspec_makescipix(tmparcimg, tset_slits, arc2=tmparcimg, CHK=chk, /THAR)
              endelse
				...
				if size(piximg, /type) EQ 2 then begin
	         	...
	         	continue
                                ...
                                ; Fit the 2D sky
              skyimage = tspec_skymodel(sciimg=sciimg, sciivar=sciivar, piximage=piximg $
                                       ,tset_slits=tset_slits)
