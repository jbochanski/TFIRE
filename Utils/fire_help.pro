PRO FIRE_HELP, SETUP=setup, TRACE=trace, FLATS=flats, STRUCTURE=structure, EXTRACT=extract, TELLURIC=telluric, COMBINE=combine, QUICKLOOK=quicklook, HOTKEYS=hotkeys, WIDGET=widget, BOTH=both, _EXTRA = keys
	...
	if keyword_set(STRUCTURE) then begin
		...
		msg = [ msg, "    Run script (and Save):  Runs a user-generated script which contains information on changes to make.  The script should be called 'firestrct_script.txt' and located in your current directory.  See $FIRE_DIR/Doc/firestrct_script.txt for documentation/an example of such a script." ]
