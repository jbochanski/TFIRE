...
function t_fitpixarc, arc_fil, ordr_str, piximg, orderimg, out_fil, $
  ...
  func_name = 't_fitpixarc'
  if  N_params() LT 3  then begin 
      print,'Syntax - ' + $
        'a = t_fitarc(arc_fil, ordr_str, out_fil, ' + $
  ...
  print, 't_fitpixarc: Reading arc: ', arc_fil
...
;         endif
         ...
         if (ordervec[iord] EQ 7) then begin
            sv_aspec[0:1024,iord] =  0
         endif
...
;         sv_aspec = tmp_aspec
         ...
         for ilin=0, nlin-1 do begin
            ...
            cent = f_centspln(x, sv_aspec[x,ii])
...
;              stop
	  ...
	  if ngd lt 2 then begin
          ...
          if not keyword_set( PINTER ) then begin ;; AUTO FIT
             ...
             x_identify, sv_aspec[*,ii], id_fit, LINELIST=strtrim(linlist,2), $
