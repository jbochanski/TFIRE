function tspec_arc2, DATA=data, ARCRAW=arcraw, ordr_str=ordr_str, $
               ...
               ORDERMASK=ordermask, OH=oh, THAR=thar, STD=std, NOPLOT=noplot, $
               CHECK_INPUTS=check_inputs, REDUXPATH=reduxpath, OBJNAME=objname, ERR_MESSAGE=err_message

  func_name = "tspec_arc2.pro"
  ...
  if keyword_set(DATA) then begin
     if NOT keyword_set(ARCRAW) then begin
        arcraw = tspec_get(data,/WAVECALFILE,/ONEARC,STD=std,OH=oh,THAR=thar)
     endif
     if NOT keyword_set(ordr_str) then ordr_str = mrdfits(tspec_get(data,/ordr_str_file),1)
     if NOT keyword_set(ARCFITS) then arcfits = tspec_get(data,/ARCFITS,STD=std,OH=oh,THAR=thar)
     if NOT keyword_set(arcimg_fil) then arcimg_fil = tspec_get(data,/ARCIMGFITS,STD=std,OH=oh,THAR=thar)
  endif
  ...
  if (slit EQ 0) then begin ; Header problems
     slit = 1.1 ; Give up and use a reasonable default -- changed for Tspec
  endif

  if (keyword_set(OH)) then begin
     pkwdth=(slit/0.52)*1.5d ;; pixels per slit times 1.2
...
;     psig[20]=50.0
  ...
  tspec_proc, arcraw, arcframe, arcinvvar, hdr=hdr;, /MASKBADPIX

  tset_slits = xmrdfits(ordermask, 1)
  orderimg = tspec_ordermask(tset_slits)
  ...
  if (strtrim(sxpar(hdr,"LAMP"),2) EQ "ThAr") then begin
     piximg = tspec_makescipix(arcframe, tset_slits, arc2=arcframe, chk=chk, /ThAr)
  endif else begin
     piximg = tspec_makescipix(arcframe, tset_slits, arc2=arcframe, chk=chk)
  endelse
...
;  endif
  ...
  if (not (keyword_set(GUESSARC)) AND not (keyword_set(INTER))) then begin
     fire_dir = getenv("FIRE_DIR")
     if (keyword_set(OH)) then $
        guessarc = strtrim(fire_dir,2)+'/Arcs/TSpec_jjb_guess2.idl'
     if (keyword_set(THAR)) then $
  ...
  arcimg = fltarr(1024, 2048);fltarr(2048, 2048)
