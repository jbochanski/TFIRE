...
PRO FIRE_LONG_COMBSPEC, influx, inivar, loglam $
...
FOR j = 0L, nimgs-1L DO BEGIN
    ...
    IF dims_lam EQ 1 THEN BEGIN
        ...
        IF NOT KEYWORD_SET(NOSHARP) THEN BEGIN
            spix = WHERE(mask1, nspix)

				if nspix EQ 0 then begin
					fire_siren, func_name + ": ERROR with mask!  Exiting!"
					flag = 1
					err_message = "error with mask in " + func_name
					RETURN
				endif else begin
...
;;;;;;;Change 2: exclude the points we have already set to zero
        ...
        sclflux[in_lam, j]  = interpol(flux1, loglam[*, j], newloglam[in_lam]) $
          *(sclmask[in_lam, j] EQ 0)
;        sclflux[in_lam, j] = abs(sclflux[in_lam, j])
...
FOR j = 0L, nimgs-1L DO BEGIN
    ...
    ind_nonzero = where(sclivar[in_lam, j] GT 0 AND sn2 GT 0, nnozero)
    IF nnozero GT 0 THEN djs_iterstat, sn2[ind_nonzero], mean = mean_sn21 $
    ...
    med_width = round(nin/((max(newloglam[in_lam])-min(newloglam[in_lam]))/ $
                           bkspace))
    mask_pts = where( bsp_mask, npts )
    if npts EQ 0 then begin
    	err_message = "Image " + fire_string(j+1) + " of " + fire_string(nimgs) + $
    		" has no bsp_mask"
    	fire_siren, func_name + ": ERROR!  " + err_message + "!  Exiting!"
    	flag = 1
    	err_message = err_message + " in " + func_name
    	RETURN
    endif                       
    med_width = (med_width < n_elements(where(bsp_mask)))
    sn2_med = fltarr(nin)
    sn2_med1 = djs_median(sn2[WHERE(bsp_mask)], width = med_width $
                          , boundary = 'reflect')
    sn2_med2 = interpol(sn2_med1, newloglam[in_lam[WHERE(bsp_mask)]] $
                        , newloglam[in_lam])
    ...
    ;sn2_med[where(bsp_mask)] = sn2_med1 > 0.0
    ;sn2_set = bspline_iterfit(newloglam[in_lam], sn2_med, ivar = bsp_mask $
    ;                          , upper = 2.0, lower = 2.0 $
    ;                          , bkspace = bkspace, yfit = sn2_fit)
    ;sn2_weights_temp = djs_median(sn2[ind_nonzero], width = round(nspec/5) $
    ;                              , boundary = 'reflect')
    ;sn2_weights[*, j] = interpol(sn2_weights_temp, newloglam[ind_nonzero] $
    ;                             , newloglam)
...
FOR j = 0L, nimgs-1L DO BEGIN
   IF avg_sn LE 0.5 THEN BEGIN 
       arrflux[*, j] = sclflux[*, j]
       arrivar[*, j] = sclivar[*, j]
       arrmask[*, j] = sclmask[*, j]
       arrsky[*, j]  = sclsky[*, j]
       arrnivar[*, j] = sclnivar[*, j]
  ENDIF ELSE IF avg_sn LE 2.0 OR  KEYWORD_SET(MEDSCALE) THEN BEGIN
   ...
   ENDIF ELSE BEGIN
      ...
      plot, xvector, sclflux[*, j] > (-100.0)
      oplot, xvector, (refflux > (-100.0D)), color=colors.red
;stop
      solve_poly_ratio, xvector, (sclflux[*, j] > (-100.0)) $
        ...
        IF NOT KEYWORD_SET(NOSHIFT) AND avg_sn GE 7.0 THEN BEGIN
            ...
            IF abs(xpeak) GT 1.5 THEN message $
              , 'Your spectra are not aligned to within 1.5 pixels' $
            ELSE IF abs(xpeak) LT 0.05 THEN $
...
;    ELSE sigrej_final = sigrej
    ...
    FOR ii = 0L, niter-1L DO BEGIN
        ...
        FOR j = 0L, nimgs-1L DO BEGIN
            ...
            chi2  = (arrflux[*, j]-newflux_now)^2*ivar_real
...
IF KEYWORD_SET(CHECK) THEN BEGIN
    loadct, 0
    color_vec = lonarr(10)
    color_vec[0] = fsc_color('white', 255)
    color_vec[1] = fsc_color('red', 240)
    color_vec[2] = fsc_color('green', 241)
    color_vec[3] = fsc_color('blue', 242)
    color_vec[4] = fsc_color('cyan', 243)
    color_vec[5] = fsc_color('magenta', 244)
    color_vec[6] = fsc_color('yellow', 245)
    color_vec[7] = fsc_color('orange', 246)
    color_vec[8] = fsc_color('purple', 247)
    color_vec[9] = fsc_color('tan', 248)
