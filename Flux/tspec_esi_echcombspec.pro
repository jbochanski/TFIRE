...
pro tspec_esi_echcombspec, allobj, echfspec, SILENT=silent $
                     , LIST=list $
...
;  Optional Keywords
  if not keyword_set(ORDRS) then ordrs=[3L,7L];[11L,31L]  ;removed the orders from the call
...
;  for i=0L,nspec-1 do echfspec.texp[i] = allobj[all_zero[i]].exp
  ...
  for qq=ordrs[1],ordrs[0],-1 do begin
      all_ordr = where(allobj.order EQ 7-qq)
...
;         oplot, 10^inloglam, influx[*,3]
         ...
         fire_long_combspec, influx, inivar, inloglam, insky = insky $
			...
			if FLAG NE 0 then begin
				fire_siren, func_name + ": ERROR with fire_long_combspec!  Exiting prematurely!"
				RETURN
			endif                           
