...
pro fire_telluric_combine, state
  ...
  if (n_elements(tel_indx) EQ 1) then begin
     telluric = telobjs[tel_indx[0]]
  		...
  		telluric = tspec_stdcombine( filelist, sigrej=3, /chk )  
  endelse
...
pro fire_telluric, fire, objstrfile=objstrfile, CLOBBER=clobber, NAME=name, QUICK=quick
...
;;;; choose which exposures to use.
  ...
  if (num_tells GT 1) then begin
     ...
     for itel=0, num_tells-1 do begin
        telobjs[itel] = tspec_get(fire[struct_entry[0]],/TOBJSTRFILE, std=itel+1)
...
;        matchstr = "Object/Tel*"+strtrim(filenum,2)+"*"
     ...
     telobjs[0] = tspec_get(fire[struct_entry[0]],/TOBJSTRFILE, std=1)
     telluric   = telobjs[0]
  endelse
  ...
  if (x_chkfil(tellcorfile, /SILENT) EQ 0) OR keyword_set(CLOBBER) then begin
     ...
     tspec_xtellcor, object=tmpobj, userpath="Object/", telluric=telluric, bmag=B, vmag=V, outfile=outfile1, name=name
  endif else begin
