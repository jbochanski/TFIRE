pro tspec_guicombine, fire, targs=targs, groupleader=groupleader, WIDGET=widget, _EXTRA = keys, ORDERS=orders

	func_name = 'tspec_guicombine'
  ...
  for itarg=0, ntargs-1 do begin
     ...
     for imatch=0, nmatch-1 do begin        
        ...
        objstr_filenames[imatch] = tspec_get_fitnum(objstr_file)
     ...
     if( first EQ 1 ) then begin
	     ...
	     tspec_combspec, all_objstr, fspec, FLAG=flag, ERR_MESSAGE=err_message
	     if FLAG NE 0 then begin
	     		if is_undefined(err_message) EQ 1 then err_message = "None provided."
	     		fire_siren, func_name + ": FAILURE combining " + targs[itarg] + "!  (Error message: " + err_message + ")  Skipping this target's correction.", WIDGET=widget, /both, /append
	     		used(*) = 0     	 
	     endif else begin
	     		tspec_1dspec, fspec, outfile, sigfile, hdr=output_hdr, /zero
