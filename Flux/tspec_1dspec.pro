...
pro tspec_1dspec,spec,outfile,sigfil,combname, resvel=resvel $
                , REBIN=rebin, hdr=hdr, CHK=chk, ZERO=zero, ADD=add

	func_name = 'tspec_1dspec'
...
;;; begin 1d
  ...
  for i=0L,nuse-1L do begin
     cnst = 66000;26402.6  ;order * central wavelength
                                ;cnst = 61700.
...
;  boxcar extraction.
  ...
  sg = gd[sort(sw[gd])]
  min_ii = (where(abs(tot_wave-min(sw[gd])) LT 1))[0]
  max_ii = (where(abs(tot_wave-max(sw[gd])) LT 1))[0]
...
;    oplot, tot_wave[a], tot_flux[a], color=colors.green
  ...
  if (keyword_set(ZERO)) then begin
     ...
     notrans = where((tot_wave GT 13500 AND tot_wave LT 14800) OR $
                     (tot_wave GT 17900 AND tot_wave LT 19300), ntrans)
