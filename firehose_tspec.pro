...
;    stop
 ...
 'go_combine': BEGIN
    ...
    fire_timer, time, /start  
    tspec_guicombine, fire, targs=targs, WIDGET=s.wtext, /append, ORDERS=multispec_out
    fire_timer, time, /stop
 ...
 'go_pipe': BEGIN
    ...
    fire_timer, time, /start
    verbose = WIDGET_INFO(s.fire_pipe_verbosebt,/button_set)
    chk = WIDGET_INFO(s.fire_pipe_chkbt,/button_set)
    bright = WIDGET_INFO(s.fire_pipe_brightbt,/button_set)
    clobber = WIDGET_INFO(s.fire_pipe_clobberbt,/button_set)
    plot_info = WIDGET_INFO(s.fire_pipe_plotbt,/button_set)
    tspec_pipe, fire, reduxpath=reduxpath, targets=runtargs, Verbose=verbose, CHK=chk, BRIGHT=bright, CLOBBER=clobber, PLOT_INFO=plot_info, WIDGET=s.wtext, /append
...
;     looking for an objstr file
  ext_index = where(firestrct.obj_id GE 0 AND $
                    FILE_TEST(firestrct.objstrfile) EQ 1, nextracted)
...
;    Build the tree in the Telluric tab        
     for iext=0, next-1 do begin
        ...
        for imatch=0, nmatch-1 do begin
           ...
           leafname = (strsplit(objstr,"/", /extract))[1]
     ...
     if next GT 0 then begin
        s.tell_branches = tspec_names_to_list( tell_branches, /double )
     endif
...
;  flatls = (strsplit(flatls,'/',/extract))[1]
;  illumls = (strsplit(illumls,'/',/extract))[1]
;  ordrls = (strsplit(ordrls,'/',/extract))[1]
  ...
  wt7c = WIDGET_BASE(wt7, /nonexclusive, /row)
...
;  state = {cwd:cwd, wtext:wtext, setupdir:setupdir, tflt_file:tflt_file, greenfile:greenfile, redfile:redfile, pixarcfile:pixarcfile, piximgfile:piximgfile, pixarcinterbt:pixarcinterbt, redflatfiles:redflatfiles, greenflatfiles:greenflatfiles, blueflatfiles:blueflatfiles, illumflatfiles:illumflatfiles, flatfile:flatfile, strctname:strctname,  stdfile:stdfile, stdarcfile:stdarcfile, stdflxfile:stdflxfile, stdoutfile:stdoutfile, w_spec_target:w_spec_target, qlfile:qlfile, bquit:bquit}
  ...
  xmanager, 'fire_tspec', main , /no_block      ; wait for events
END
