...
pro long_slitmask_add1, i, xpos, xerr, xpos_add, xerr_add
...
;------------------------------------------------------------------------------
pro fire_slitmask, filename, outfile, biasfile = biasfile $
...
fire_proc, filename, image, ivar $
           , hdr = hdr, biasfile = biasfile
    

image = reverse(transpose(image))
...
ELSE IF sxpar(hdr[*,0], 'XGMOS') AND KEYWORD_SET(GMOSLONG) then begin 
...
;; Fit these initial traces by a polynomial function
   ...
   if strcmp(instrument, 'LRISBLUE') OR $
      ...
      if (ct EQ 1) then begin
         ...
         long_slitmask_add1, i+1, xpos1, xerr1, xpos1add, 0*xpos1add+1.
         long_slitmask_add1, i, xpos2, xposivar, xpos2add, 0*xpos2add+1.
         nslit = nslit + 1L
      endif
  endif
   ;stop
...
;stop
  ...
  ;stop
  xy2traceset, rebin(findgen(ny), ny, nslit), xpos1, sset1 $
               , func = func, ncoeff = ncoeff, invvar = xposivar
  xy2traceset, rebin(findgen(ny), ny, nslit), xpos2, sset2 $
               , func = func, ncoeff = ncoeff, invvar = xposivar
