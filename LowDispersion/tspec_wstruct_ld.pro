...
;-
;------------------------------------------------------------------------------
FUNCTION tspec_WSTRUCT_ld, hdr, LINELIST=linelist, REID_FILE=REID_FILE

;;-----------
;;  Line list directory
calib_path =  GETENV('FIRE_DIR') + '/LowDispersion/' ;GETENV('LONGSLIT_DIR') + '/calib/linelists/'
line_path = GETENV('FIRE_DIR') + '/Arcs/'
;;----------
...
ENDIF ELSE IF strmatch(telescope, 'Gemini-North') THEN BEGIN
 ...
 ENDIF ELSE IF (instrument EQ 'FIRE') then begin
    ...
    wstruct.REID_FILE     = getenv("FIRE_DIR")+'/LowDispersion/near_fed5.sav'
    wstruct.func          = 'CHEBY'
    wstruct.nord           = 6
ENDIF  ELSE IF (instrument EQ 'tspec') then begin
    wstruct.pkwdth        = 12.0
    wstruct.TOLER         = 4.0D
    wstruct.psig[0:4]     = [25.0, 18.0, 10.0, 7.0, 4.0]
    wstruct.MXOFF[0:4]    = [8.0, 8.0, 6.0, 6.0, 4.0]
    ;; paramters for wavelength soln fits
    wstruct.sigrej[0:4]   = [3.0, 3.0, 3.0, 3.0, 3.0]
    wstruct.nord[0:4]     = [6L, 6L, 6L, 6L, 6L]
    wstruct.FLG_QUAL[0:4] = [1L, 1L, 1L, 1L, 2L]
    wstruct.LINELIST  = line_path+'/FIRE_OH_R6000.lst'
    wstruct.npanic        = 5L
    wstruct.nord_panic    = 2L
    wstruct.REID          = 1   ; reidentify using archived solutions
    wstruct.mxshift       = 50.0
    wstruct.REID_FILE     = getenv("FIRE_DIR")+'/Arcs/TSpec_guess.idl'
    wstruct.func          = 'CHEBY'
    wstruct.nord           = 6
    ENDIF ELSE BEGIN
