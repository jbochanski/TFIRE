...
;;;;;;;;;
 ...
 common x_identify_fit
   ...
   lines = state.lines
   x_templarc, state.fx, lines, tcalib,  FORDR = FORDR $
...
common x_identify_fit
  ...
  ;; Orig wave
  wav = x_calcfit(dindgen(mtch_npix), FITSTR=mtch_fitstr)
...
common x_identify_fit
  ...
  if median(dwv) LT 0 then dwv = (-1)*dwv 
  a = where(dwv LT 0,na)
...
;;; LINELISTS
  ...
  if keyword_set( LINELIST ) then begin
     ...
     a = [a,linelist]
     init = where(strtrim(linelist,2) EQ state.lists, count)
...
;;;; GUESS
  ...
  mtchbase = WIDGET_BASE(toolbar, /column, /base_align_center, /frame, $
                     /align_center)
  state.shift_id = CW_FIELD(mtchbase, value=0, xsize=5, ysize=1, $
                            title='SHIFT', $
                            /return_events, uvalue='SHIFT')
  state.stretch_id = CW_FIELD(mtchbase, value=0, xsize=5, ysize=1, $
...
; INLIN
  if keyword_set(INLIN) then begin
     for ilin=0, n_elements(inlin)-1 do begin
;<<<<<<< x_identify.pro
;        match = where(inlin[ilin].wv EQ state.lines.wave)
;        state.lines[match[0]].flg_plt = inlin[ilin].flg_plt
;        state.lines[match[0]].pix = inlin[ilin].pix
;        state.lines[match[0]].wave_fit = inlin[ilin].wave_fit
;        stop
;=======
        match = where(inlin[ilin].wave EQ state.lines.wave)
        state.lines[match].flg_plt = inlin[ilin].flg_plt
        state.lines[match].pix = inlin[ilin].pix
        state.lines[match].wave_fit = inlin[ilin].wave_fit
;        stop
;>>>>>>> 1.3
...
;  Wavelength array
  ...
  ;; OUTLIN (good lines)
  if keyword_set(GD_LINES) then outlin = temporary(gd_lines)
