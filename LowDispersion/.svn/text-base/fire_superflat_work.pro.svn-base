...
;-
;------------------------------------------------------------------------------
pro fire_superflat_work, filenames, superpixflatfile, superillumflatfile $
                    , slitfile = slitfile $
                    ...
                    , CHK = CHK, SLITSAMP=slitsamp,NIRSPEC=NIRSPEC
  ...
  if total(use_illum1) + total(use_pixel1) EQ 0 then return
   ...
   IF NOT KEYWORD_SET(EDGTOL) THEN EDGTOL = 2.5D
...
;     Read this file
       IF KEYWORD_SET(NIRSPEC) THEN $
          nirspec_proc, filenames[ifile] $
                        , image1, invvar1, hdr = hdr1, darkfile = biasfile $
                        , verbose = verbose $
       ELSE fire_proc, filenames[ifile] $
                       , image1, invvar1, hdr = hdr1, biasfile = biasfile $
                       , verbose = verbose 
       image1 = reverse(transpose(image1))
       invvar1 = reverse(transpose(invvar1))
...
;         If slitfile is provided generate slitmask 
...
;         if wavefile is provided generate piximag and waveimg
...
;        Default to all mask values of 1B, which rejects points
      ...
      IF strcmp(telescope, 'KeckI') OR KEYWORD_SET(NIRSPEC) THEN BEGIN
         ...
         TOL_EDG = 4.0 >  1.5*abs(XSHIFT)
      ENDIF ELSE TOL_EDG = 4.0D
