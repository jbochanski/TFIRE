...
common xatv_state, state, tot_state, nimg, flg_img, xout, yout, xfit, yfit
...
;  Register
    'R': xatv_register
    'L': xatv_matchluts
;  Outputting XY coords
    ' ': xatv_xyouts, event
    'P': xatv_guessprof, event
    'C': xatv_clearprof, event
...
if (xout[0] EQ -1 OR yout[0] EQ -1) then begin
   main_image1 = main_image
   xout = state.coord[0]
   yout = state.coord[1]
endif else begin
   xout = [xout, state.coord[0]]
   yout = [yout, state.coord[1]]
;   atvplot, xout, yout
endelse
main_image[state.coord[0]-5:state.coord[0]+5, state.coord[1]] = -10000
main_image[state.coord[0],state.coord[1]-5:state.coord[1]+5] = -10000
...
if (n_elements(xout) GT 1 AND n_elements(yout) GT 1) then begin
   coeffs = poly_fit(yout, xout, 1)
   xfit = poly(findgen(2048), coeffs)
   main_image[xfit, findgen(2048)] = -10000
endif

if (n_elements(xout) EQ 1 AND n_elements(yout) EQ 1) then begin
   xfit = fltarr(2048) + xout
   main_image[xfit, findgen(2048)] = -10000
endif
...
if (n_elements(xfit) EQ 2048) then begin
   retx = xfit
   rety = findgen(2048)
endif else begin
   if (n_elements(xout) GT 0) then begin
      retx = xout
      rety = yout
   endif
endelse
