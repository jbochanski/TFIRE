...
;
; PURPOSE:
;     Displays SpeX spectral FITS data.
;    
; CATEGORY:
;     Widget
;
; CALLING SEQUENCE:
;     xvspec,[file],CANCEL=cancel
;     
; INPUTS:
;     None
;    
; OPTIONAL INPUTS:
;     file -  A string giving the filename of a SpeX spectral FITS image
;
; KEYWORD PARAMETERS:
...
state.w.xvspec_base = widget_base(TITLE='Xvspec', $
   ...
   button = widget_button(state.w.xvspec_base,$
                          FONT=buttonfont,$
                          VALUE='Done',$
                          EVENT_PRO='xvspec_event',$
                          UVALUE='Done')

   state.w.keyboard = widget_text(state.w.xvspec_base, $
                                  /ALL_EVENTS, $
                                  SCR_XSIZE=1, $
                                  SCR_YSIZE=1, $
                                  UVALUE='Keyboard', $
   ...
   row_base = widget_base(state.w.xvspec_base,$
                          /ROW,$
                          EVENT_PRO='xvspec_event')
...
openr, lun, filepath('xvspec_helpfile.txt',ROOT_DIR=state.r.packagepath, $
                     SUBDIR='helpfiles'),/GET_LUN
nlines = numlines(filepath('xvspec_helpfile.txt',$
...
xmc_displaytext,array,TITLE='Xvspec Help File',GROUP_LEADER=state.w.xvspec_base

end
;
;******************************************************************************
;
pro xvspec_loadspec,fullpath
...
endif else fire_readspec,fullpath,spectra,hdr,obsmode,start,stop,norders,naps, $
...
widget_control, state.w.xvspec_base, $
                TLB_SET_TITLE='Xvspec - '+ $
...
case uvalue of
    ...
    'Keyboard': begin
        ...
        if strtrim(event.ch,2) eq 'p' then xvspec_plotupdate,/PS
...
;******************************************************************************
;
pro xvspec,file,CANCEL=cancel

if not xregistered('xvspec') then xvspec_startup

common xvspec_state

if n_params() eq 1 then begin

    cancel = cpar('xvspec',file,1,'File',7,0)
    if cancel then return
    file = cfile(file,WIDGET_ID=state.w.xvspec_base,CANCEL=cancel)    
    if cancel then delvarx,file

endif

xvspec_loadspec,file
xvspec_plotupdate
