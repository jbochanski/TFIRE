...
tmp = std_flux[sort(std_flux[where(std_flux GT 0)])]
mx  = tmp[0.95 * n_elements(tmp)]
...
p = {absxrange1:[min(std_wave,max=max),max],$
     ...
     xrange1:[min(std_wave[where(std_wave GT 0)],max=max),max],$
     xrange2:[0.,0.],$
     xscale1:!x,$
     xscale2:!x,$
     xtitle:xtitle,$
...
tmp = state.d.nflux[sort(state.d.nflux)]
ntmp = n_elements(tmp)
mn  = tmp[0.02 * ntmp] - 0.2
mx  = tmp[0.98 * ntmp] * 1.2

;state.p.yrange2 = [min((state.d.nflux)[idx[0]:idx[1]]-1,max=ymax,/NAN),ymax]
state.p.yrange2 = [mn,mx]
          ...
          subrow_base = widget_base(row_base)
             ...
             state.w.norminfo_base = widget_base(subrow_base,$
                ...
                fitorder_dl = widget_droplist(state.w.norminfo_base,$
                                              ...
                                              VALUE=value,$
                                              UVALUE='Fit Degree')
                WIDGET_CONTROL, fitorder_dl, set_droplist_select=5
                state.r.normdegree = 6
