...
case uvalue of
    ...
    'Help': xtellcor_help,state

    'Load Spectra': begin
       xtellcor_loadspec,state
    end
    ...
    'Path Button': begin
        ...
        path= dialog_pickfile(/DIRECTOR,DIALOG_PARENT=state.w.xtellcor_base,$
                              TITLE='Select Path',/MUST_EXIST)
        
        if path ne '' then begin

            path = cpath(path,WIDGET_ID=state.w.xtellcor_base,CANCEL=cancel)
            if cancel then return
...
;******************************************************************************
;
pro xtellcor_changeunits,wave,tellspec,tellspec_error,state
...
for i = 0, norders-1 do begin
    ...
    telluric, (*state.d.stdspec)[*,0,z],(*state.d.stdspec)[*,1,z],$
      ...
      (*state.d.kernels).(i),(*state.r.scales)[*,z],*state.d.wvega,$
...
;  Perform interpolations if necessary
    ...
    xtellcor_changeunits,(*state.d.stdspec)[*,0,z],tellcor,tellcor_error,state
...
xscalelines,(*state.d.stdspec)[*,*,stdidx],(*state.d.stdorders)[stdidx],$
            ...
            *state.d.cf2vega,*state.d.kernels,vshift,(*state.d.objspec)[*,*,objidx], $
            ...
            XPUTS=xputs,PARENT=state.w.xtellcor_base,XTITLE=state.r.xtitle, $
            YTITLE=state.r.ytitle,CANCEL=cancel
...
;******************************************************************************
;
pro xtellcor_help,state

openr, lun, filepath('xtellcor_helpfile.txt',ROOT_DIR=state.r.packagepath, $
                     SUBDIR='helpfiles'),/GET_LUN
nlines = numlines(filepath('xtellcor_helpfile.txt',$
...
xmc_displaytext,array,GROUP_LEADER=state.w.xtellcor_base,CANCEL=cancel
if cancel then return
...
if cancel then return
std = cfile(path+std,WIDGET_ID=state.w.xtellcor_base,CANCEL=cancel)
if cancel then return
...
if cancel then return
obj = cfile(path+obj,WIDGET_ID=state.w.xtellcor_base,CANCEL=cancel)
if cancel then return
...
;Changing this to interface with the XIDL pipeline ... dump spectra
;back into the object structure
...
;;  Get info from obj and std hdr.
     ...
     (*state.d.stdhdr) = headfits('../Raw/fire_'+splitarr[n_elements(splitarr)-1])
     (*state.d.objhdr) = objhdr

     norders   = n_elements(obj_strct)
     start     = min(obj_strct.slitid)
     stop      = max(obj_strct.slitid)
     orders    = strtrim(start,2)+','+strtrim(stop,2)
     obsmode   = fxpar(*state.d.stdhdr,'GRISM',cobsmode)
     std       = fxpar(*state.d.stdhdr,'OBJECT',cobj)
     xunits    = 'Angstroms'
...
;;  Create hdr for the telluric correction spectrum
     ...
     fxaddpar,hdr,'STOP',stop, 'stopping order' 
...
; hack by AJB to put in order shifts
     for i=start,stop do fxaddpar,hdr,'SHIFT'+strtrim(string(i),2),(*state.r.shift)[i-start]
...
;; sxaddhist,' ',hdr
;; sxaddhist,'Xtellcor History',hdr
;; sxaddhist,' ',hdr
...
;******************************************************************************
;
pro xtellcor, object=object, userpath=userpath, telluric=telluric, $
...
state.w.xtellcor_base = widget_base(TITLE='Xtellcor', $
                                    EVENT_PRO='xtellcor_event',$
                                    /COLUMN)
    
   quit_button = widget_button(state.w.xtellcor_base,$
                               FONT=buttonfont,$
                               VALUE='Done',$
                               UVALUE='Quit')
       
   state.w.message = widget_text(state.w.xtellcor_base, $
                                 VALUE='',$
                                 YSIZE=1)
...
centertlb,state.w.xtellcor_base
