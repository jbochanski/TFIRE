...
;    'Construct Kernel': xtellcor_fire_conkernel,state
    
    'Construct Telluric Spectra': xtellcor_fire_contellspec,state

    'Get Shift': xtellcor_fire_getshift,state

    'Help': xtellcor_fire_help,state

    'Load Spectra': xtellcor_fire_loadspec,state
    ...
    'Object Spectra Button': begin
        ...
        obj = dialog_pickfile(DIALOG_PARENT=state.w.xtellcor_fire_base,$
    ...
    'Path Button': begin
        ...
        path= dialog_pickfile(/DIRECTORY,$
                              DIALOG_PARENT=state.w.xtellcor_fire_base,$
                              TITLE='Select Path',/MUST_EXIST)
        
        if path ne '' then begin

            path = cpath(path,WIDGET_ID=state.w.xtellcor_fire_base,$
                         CANCEL=cancel)
            if cancel then return
    ...
    'Scale Lines': xtellcor_fire_getscales,state
    ...
    'Standard Spectra Button': begin
        ...
        path = cfld(state.w.path_fld,7,CANCEL=cancel)
        if cancel then return
        std = dialog_pickfile(DIALOG_PARENT=state.w.xtellcor_fire_base,$
                             PATH=path,/MUST_EXIST,FILTER='*dat')
    ...
    'Write File': xtellcor_fire_writefile,state
...
cont: 
widget_control, state.w.xtellcor_fire_base, SET_UVALUE=state, /NO_COPY
getout:
...
;******************************************************************************
;
pro xtellcor_fire_cleanup,base
...
;******************************************************************************
;
pro xtellcor_fire_changeunits,wave,tellspec,tellspec_error,state
...
;******************************************************************************
;
pro xtellcor_fire_conkernel,state
...
;******************************************************************************
;
pro xtellcor_fire_contellspec,state
...
xtellcor_fire_changeunits,(*state.d.stdspec)[*,0],tellcor,tellcor_error,$
  state
...
;******************************************************************************
;
pro xtellcor_fire_getscales,state
...
xscalelines,*state.d.stdspec,state.d.stdnorders,state.r.vmag,$
  ...
  *state.d.kernels,vshift,*state.d.objspec,[1],1,state.d.awave,$
  state.d.atrans,state.r.hlines,state.r.hnames,state.r.scale,$
            ...
            PARENT=state.w.xtellcor_fire_base,$
            CANCEL=cancel
...
;******************************************************************************
;
pro xtellcor_fire_help,state

    
openr, lun, filepath('xtellcor_fire_helpfile.txt',$
                     ROOT_DIR=state.r.packagepath,SUBDIR='helpfiles'),$
       /get_lun
nlines = numlines(filepath('xtellcor_fire_helpfile.txt',$
...
xmc_displaytext,array,TITLE='Xvspec Help File', $
                GROUP_LEADER=state.w.xtellcor_fire_base

end
;
;******************************************************************************
;
pro xtellcor_fire_loadspec,state
...
if cancel then return
std = cfile(path+std,WIDGET_ID=state.w.xtellcor_fire_base,CANCEL=cancel)
if cancel then return
...
if cancel then return
obj = cfile(path+obj,WIDGET_ID=state.w.xtellcor_fire_base,CANCEL=cancel)
if cancel then return
...
readspec_jjb,std,sspec,stdhdr,stdnorders,stdnaps,stdorders,$
         slitw_pix,slitw_arc, $
         stdairmass


readspec_jjb,obj,ospec,objhdr,objnorders,objnaps,objorders,$
         slitw_pix,slitw_arc, $
         objairmass
...
;******************************************************************************
;
pro xtellcor_fire_getshift,state
...
shift = fire_xfindshift(obj_wave,obj_flux,new_flux,INITSHIFT=state.r.shift,$
                   XTITLE='!5!7k!5 (!7l!5m)', $
                   PARENT=state.w.xtellcor_fire_base,$
                   CANCEL=cancel)
...
;******************************************************************************
;
pro xtellcor_fire_writefile,state
...
state.w.xtellcor_fire_base = widget_base(TITLE='Xtellcor_FIRE', $
                                    EVENT_PRO='xtellcor_fire_event',$
                                    /COLUMN)
    
   quit_button = widget_button(state.w.xtellcor_fire_base,$
                               FONT=buttonfont,$
                               VALUE='Done',$
                               UVALUE='Quit')
       
   state.w.message = widget_text(state.w.xtellcor_fire_base, $
                                 VALUE='',$
                                 YSIZE=1)

   row_base = widget_base(state.w.xtellcor_fire_base,$
                          /ROW)
...
;                                   FONT=buttonfont)
   ...
   help = widget_button(state.w.xtellcor_fire_base,$
                        VALUE='Help',$
                        UVALUE='Help',$
                        FONT=buttonfont)
...
centertlb,state.w.xtellcor_fire_base
widget_control, state.w.xtellcor_fire_base, /REALIZE

; Start the Event Loop. This will be a non-blocking program.

XManager, 'xtellcor_fire', $
  state.w.xtellcor_fire_base, $
  CLEANUP='xtellcor_fire_cleanup',$
  /NO_BLOCK
