...
;;  Get info from obj and std hdr.
     ...
     what = splitarr[n_elements(splitarr)-1];JJB hack to deal with case of "raw"
     name = sjs_getname(what);'./raw/fire_'+splitarr[n_elements(splitarr)-1]
  ; IF file_test(name) EQ 1 THEN (*state.d.stdhdr) = headfits(name) ELSE  (*state.d.stdhdr) = headfits('../Raw/fire_'+splitarr[n_elements(splitarr)-1]) 
     (*state.d.objhdr) = objhdr
...
;     obsmode   = fxpar(*state.d.stdhdr,'GRISM',cobsmode)
     std       = name;fxpar(*state.d.stdhdr,'OBJECT',cobj)
     xunits    = 'Angstroms'
...
;******************************************************************************
;
pro tspec_xtellcor, object=object, userpath=userpath, telluric=telluric, $
